...
Run Trial 3 with improved initialization to avoid death spirals.
"""
...
async def run_config2_trial3():
    """
    Run Config 2 trial 3 with seed 3 and improved initialization.
    """
    print("=" * 80)
    print("CONFIG 2 TRIAL 3: Improved Initialization + Seed 3")
    print("=" * 80)
    print("\nConfiguration:")
    ...
    # Run Config 2 trial with seed 3 and improved initialization
    print("\n🚀 Running Config 2 Trial 3 (1000 episodes, seed 3, improved init)...")
    print("-" * 80)
    ...
    print(f"\nTrial 3 (1000 ep, seed 3, improved init):")
    print(f"  Final Sharpe: {pulseos_sharpe:.3f}")
    print(f"  Improvement: {improvement:+.1f}% vs PPO")
    
    # Statistics across all trials
    all_trials = [
        {"trial": "1 (prev)", "seed": "Unknown", "sharpe": 4.259, "episodes": 500},
        {"trial": "1", "seed": 1, "sharpe": 3.688, "episodes": 1000},
        {"trial": "2", "seed": 2, "sharpe": 2.053, "episodes": 1000},
        {"trial": "3", "seed": 3, "sharpe": pulseos_sharpe, "episodes": 1000}
    ]
    ...
    if pulseos_sharpe > max([t["sharpe"] for t in trials_1000ep if t["trial"] != "3"]):
    ...
    json_path = f"{output_dir}/config2_1000ep_trial3_seed3_improved_init_{timestamp}.json"
    ...
    report_path = f"{output_dir}/config2_1000ep_trial3_seed3_improved_init_{timestamp}.md"
    with open(report_path, "w") as f:
        f.write("# Config 2 Trial 3: Improved Initialization + Seed 3\n\n")
        ...
        f.write("| Trial | Episodes | Seed | Sharpe | Improvement | Status |\n")
        f.write("|-------|----------|------|--------|-------------|--------|\n")
        for t in all_trials:
            status = "✅" if t["sharpe"] > ppo_baseline_sharpe else "❌"
            f.write(f"| Trial {t['trial']} | {t['episodes']} | {t['seed']} | {t['sharpe']:.3f} | {((t['sharpe'] - ppo_baseline_sharpe) / ppo_baseline_sharpe * 100):+.1f}% | {status} |\n")
        f.write(f"\n**Test Duration**: {duration:.1f} minutes\n")
...
if __name__ == "__main__":
    asyncio.run(run_config2_trial3())
