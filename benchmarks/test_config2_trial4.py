...
Run Trial 4 with improved initialization to test another seed.
"""
...
async def run_config2_trial4():
    """
    Run Config 2 trial 4 with seed 4 and improved initialization.
    """
    print("=" * 80)
    print("CONFIG 2 TRIAL 4: Improved Initialization + Seed 4")
    print("=" * 80)
    print("\nConfiguration:")
    print("  - NO Warm Start (independent training)")
    print("  - IMPROVED Initialization:")
    print("    * Smaller initial weights (0.3x instead of 0.5x)")
    print("    * Small random bias to encourage exploration")
    print("    * More conservative value initialization")
    ...
    # Run Config 2 trial with seed 4 and improved initialization
    print("\n🚀 Running Config 2 Trial 4 (1000 episodes, seed 4, improved init)...")
    print("-" * 80)
    ...
    print("  Trial 1 (1000 ep, seed 1): 3.688 Sharpe (+1.7% vs PPO)")
    print("  Trial 2 (1000 ep, seed 2): 2.053 Sharpe (-43.4% vs PPO)")
    print("  Trial 3 (1000 ep, seed 3, improved init): 3.077 Sharpe (-15.1% vs PPO)")
    print(f"\nTrial 4 (1000 ep, seed 4, improved init):")
    print(f"  Final Sharpe: {pulseos_sharpe:.3f}")
    ...
    all_trials = [
        ...
        {"trial": "4", "seed": 4, "sharpe": pulseos_sharpe, "episodes": 1000, "init": "Improved"}
    ]
    ...
    # Filter to improved init trials
    ...
    if len(sharpes_improved_init) > 0:
        print(f"\nStatistics (improved initialization trials):")
        ...
        print(f"  Excellent (4.0+): {sum(1 for s in sharpes_improved_init if s >= 4.0)}/{len(sharpes_improved_init)}")
    
    if pulseos_sharpe > max([t["sharpe"] for t in trials_1000ep if t["trial"] != "4"]):
    ...
    json_path = f"{output_dir}/config2_1000ep_trial4_seed4_improved_init_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump({
            ...
            "statistics_improved_init": {
                ...
                "excellent_rate": sum(1 for s in sharpes_improved_init if s >= 4.0) / len(sharpes_improved_init) if len(sharpes_improved_init) > 0 else None
            },
            "duration_minutes": duration
        }, f, indent=2)
    
    report_path = f"{output_dir}/config2_1000ep_trial4_seed4_improved_init_{timestamp}.md"
    with open(report_path, "w") as f:
        f.write("# Config 2 Trial 4: Improved Initialization + Seed 4\n\n")
        f.write(f"**Date**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("## Test Configuration\n\n")
        f.write("- **Episodes**: 1000\n")
        f.write("- **Seed**: 4\n")
        f.write("- **Improved Initialization**: Yes\n")
        f.write("  - Smaller initial weights (0.3x instead of 0.5x)\n")
        f.write("  - Small random bias to encourage exploration\n")
        f.write("  - More conservative value initialization\n")
...
if __name__ == "__main__":
    asyncio.run(run_config2_trial4())
